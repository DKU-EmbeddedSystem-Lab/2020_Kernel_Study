...
//	- Search 는 Reader고, Action 들은(Insert&Delete) Writer야
// 생산 할 때는 Pre -> child 로 이어줘
// 소비 할 때는 Suc -> child를 떼어서 rm노드와 바꿔
/*
 * TODO
 *  Implement funtction of Hand-over-Hand BST in psuedo-code
 *  
 *  @param		 : bst to print in-order. 
 *  @return		 : status (success or fail)
 */

/* int pseudo-Lock-based-BST(lab2_tree *tree,arg2,...){
 *
 * 	1. Search(Reader) 
 * 	//  Reader should gain Reader-lock if this is reader0
 * 	if
 * 	or 
 * 	while(my_lock->turn == 0)
 * 		aquire_lock(this tree-level's reader)
 * 	
 * 	///Critical Section!
 *
 * 		Search_BST() ; -> Reader 0 has read-lock
 * 	
 *	///Critical Section End.
 *
 *
 * 	if(my_lock->turn == 0)
 * 		release_lock(this tree-level's reader)
 *		
 *	2. Action(Writer)
 *
 *	// for here, assume that i found a node to do
 *	// Lock Acquiring!!!
 *		
 *	// ISSUE : Why this is so hard??
 *
 *	// 굉장히 복잡한 것이, 락의 획득부분인데, 내가 소속된 락을 잡고 부모를 잡으려고 하면 환형대기가 걸린다	
 *		-> Lock 을 한 방향으로 잡아야 한다는 소리인데, 이 경우 Search에서, 내가 찾으려고 한 노드의 부모노드를 찾아 놓아야한다.(Delete)
 *		-> Insert 역시 마찬가지로 부모노드를 먼저 찾을 것이므로, 락의 획득은 Search 부분에서 굉장히 긴밀하게 잡혀야 함을 알 수 있다.
 *
 *
 *		case1 : Node->group_level != pNode->group_level ; 
 *		//부모와 해당 노드가 다른 그룹인경우
 *		
 *		case2 : (Node->group_level != Node->left->group_level || Node->group_level != Node->right->group_level)
 *		//해당 노드와 해당 노드의 자식 노드가 다른 그룹인경우
 *
 *		case3 : (Node->group_level != successor->group_level)	
 *		// Successor 계승자의 그룹과 해당 노드의 그룹이 다른그룹인경우
 *			//3.1 계승자 또한 Delete, case1,2 에 해당 될 수 있음(case2 일 경우 successor->left)
 *		if
 *		or
 *		while(this tree level's reader lock == available){	//lock
 *			aquire_lock(this tree-level's reader)	//Reader block
 *			aquire_lock(this tree-level's writer)}	//Writer block
 * 	//Lock Acquiring DOne.
 *
 *	//Critical Section
 *		if (me->myjob == 'Delete')
 ...
 *} 
 ...
 조건 변수를 사용한다
 	-> semaphore, CV
	-> SleepQueue자료구조 구현
	-> 항상 Wake 이후 Release Lock 해야한다(Avoid Deadlock) 
 * */
